...
public class TweenController : MonoBehaviour
{
    ...
    [Tooltip("The stage at which we'll start tweening from")]
    ...
    [Tooltip("List of tween stages")]
    [SerializeField] List<TweenStage> stages = new List<TweenStage>();

    private void Start()
    {
        if (beginOnStart)
            BeginStage(startingStage);
    }
    ...
    private void OnAnyTweenComplete(Tween tween)
    {
        ...
        if (tween.hideOnComplete)
        {
            if (!canvas)
            {
                ...
                if (canvas != null)
                {
                    Debug.LogWarning($"Missing Canvas on {gameObject.name}.");
                    return;
                }
            }
            if (canvas.enabled)
